...
lst<-unique(as.list(taxa_table2$Species_ed))
...
write_csv(Taxa_working, "BightTaxa.csv")

############Bold seqspec
# check4<-bold_seqspec("Eteone")
 check2<-bold_seqspec("Jassa slatteryi")
 check4<-bold_seqspec("Jassa", cleanData = T)
 check3<-bold_seqspec("Aclis")
# bold_stats("Jassa")
 ...
 for (i in 2) {
 if (class(check3)=="data.frame" && colnames(check3[1]) == "processid"){
   print("Great success")
 }
 else{
   print("Uh oh spaghetti")
 }
 }
 
 for (i in 2) {
   if (class(check4)=="data.frame" && colnames(check4[1]) == "processid"){
     print("Great success")
   }
   else{
     print("Uh oh spaghetti")
...
write_csv(FinalDF, "BOLD_Species_Check_BightTaxaFreq.csv")
...
x <- setdiff(FinalDF$FinalID, FinalDF2$FinalID)
